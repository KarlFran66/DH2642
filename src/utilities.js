...
function dishType(dish){
    ...
    else if (dish.dishTypes.find(isKnownTypeCB)===undefined){
        return "";
    }
    else{
...
function compareDishTypeCB(dishA, dishB){
    ...
    const dishTypeA=dishTypeRanking[dishType(dishA)];
    const dishTypeB=dishTypeRanking[dishType(dishB)];
    return dishTypeA-dishTypeB;
}
...
function sortDishes(dishes){
    ...
    return dishesCopy.sort(compareDishTypeCB)//TODO
}
...
// Move this function out of shoppingList() to save computing resources:
function keepJustIngredientsCB(dish){
    return dish.extendedIngredients;
}

// Replace ingredientCB with:
function amountReducerCB(accumulator, ingredient){
    if(accumulator[ingredient.id] === undefined){
        accumulator[ingredient.id]={...ingredient};
    }
    else{
        accumulator[ingredient.id].amount += ingredient.amount;
    }
    return accumulator;
}
...
function shoppingList(dishes){
    ...
    */
    ...
    return Object.values(dishes.map(keepJustIngredientsCB).flat().reduce(amountReducerCB, {}))
}
...
function menuPrice(dishesArray){
    ...
    function sumReducerCB(accumulator, element){
...
export {compareIngredientsCB, sortIngredients, isKnownTypeCB, dishType, compareDishTypeCB, sortDishes, keepJustIngredientsCB, amountReducerCB, shoppingList, menuPrice /*TODO add more here! */};
